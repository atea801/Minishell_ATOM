...
/*   Updated: 2025/11/24 13:48:11 by aautret          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * @return t_token* Pointeur vers le 2ème token de redirection
 * (en erreur), ou NULL si aucune erreur
 */
	...
	while (current && current->next && current->type)
	{
		if ((ft_strcmp(current->type, "REDIR_IN") == 0
			...
			&& current->next->type && (ft_strcmp(current->next->type,
					"REDIR_IN") == 0 || ft_strcmp(current->next->type,
					"REDIR_OUT") == 0 || ft_strcmp(current->next->type,
					"HEREDOC") == 0 || ft_strcmp(current->next->type,
					"APPEND") == 0 || ft_strcmp(current->next->type,
					"HERESTRING") == 0))
			return (current->next);
...
/**
 * @brief Vérifie la présence de tokens ERROR contenant des redirections
 * malformées
 *
 * Parcourt la liste pour trouver un token de type ERROR contenant des
 * caractères de redirection (< ou >). Si trouvé, appelle
 * print_redir_error() pour afficher e message approprié.
 *
 ...
 * Si une erreur est détectée,
	affiche le message approprié et met exit_code à 2.
 *
